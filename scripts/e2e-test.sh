...
# E2E Integration Test for Claude Code Collab
...
SERVER_PID=""
DB_PATH="/tmp/e2e-test-collab.db"
...
echo ""
echo "╔═══════════════════════════════════════════════════════════════╗"
echo "║        Claude Code Collab - E2E Integration Test              ║"
echo "╚═══════════════════════════════════════════════════════════════╝"
echo ""
...
rm -f "$DB_PATH" "$DB_PATH-shm" "$DB_PATH-wal"
DB_PATH="$DB_PATH" PORT=3847 node "$PROJECT_ROOT/server.js" > /tmp/e2e-server.log 2>&1 &
SERVER_PID=$!
wait_for_server
...
TASKS_RESPONSE=$(api GET "/teams/e2e-team/tasks")
...
UPDATE_RESPONSE=$(api PATCH "/tasks/$TASK_ID" '{"status":"in_progress"}')
...
CHAT_RESPONSE=$(api POST /chats "{\"uid1\":\"$LEAD_UID\",\"uid2\":\"$WORKER_UID\"}")
...
MESSAGES_RESPONSE=$(api GET "/chats/$CHAT_ID/messages")
...
READ_RESPONSE=$(api POST "/chats/$CHAT_ID/read" "{\"uid\":\"$WORKER_UID\"}")
...
COMPLETE_RESPONSE=$(api PATCH "/tasks/$TASK_ID" '{"status":"resolved"}')
...
echo "╠═══════════════════════════════════════════════════════════════╣"
echo "║  ✓ Server startup                                             ║"
echo "║  ✓ Team Lead authentication                                   ║"
echo "║  ✓ Worker authentication                                      ║"
